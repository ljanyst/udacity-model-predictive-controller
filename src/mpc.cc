...
class FGEval {
  public:
    ...
    void operator()(ADvector &fg, const ADvector &vars) {
      ...
      for(int i = 0; i < n_; ++i) {
        fg[0] += 0.5*CppAD::pow(vars[IND_S(S_CTE,  n_, i)], 2);
        fg[0] += 5*CppAD::pow(vars[IND_S(S_EPSI, n_, i)], 2);
        fg[0] += 0.0001*CppAD::pow(vars[IND_S(S_V,    n_, i)] - ref_speed_, 2);
      }

      // Minimize the value gap between sequential actuations.
      for(int i = 0; i < n_-2; ++i) {
        fg[0] += 3000*CppAD::pow(vars[IND_A(A_DELTA, n_, i+1)] - vars[IND_A(A_DELTA, n_, i)], 2);
        fg[0] += 1500*CppAD::pow(vars[IND_A(A_A,     n_, i+1)] - vars[IND_A(A_A,     n_, i)], 2);
      }
...
//------------------------------------------------------------------------------
MPC::Result MPC::Solve(const Eigen::VectorXd &x0,
                       const Eigen::VectorXd &trajectory) {

  //----------------------------------------------------------------------------
  // Extract the initial variables for convenience
  //----------------------------------------------------------------------------
  double x    = x0[0];
  double y    = x0[1];
  double psi  = x0[2];
  ...
  //
  // We can start with anything, except for the state in the first step,
  // so we start with 0.
  //----------------------------------------------------------------------------
  ...
  vars[IND_S(S_X,    n_, 0)] = x;
  vars[IND_S(S_Y,    n_, 0)] = y;
  vars[IND_S(S_PSI,  n_, 0)] = psi;
  vars[IND_S(S_V,    n_, 0)] = v;
  vars[IND_S(S_CTE,  n_, 0)] = cte;
  vars[IND_S(S_EPSI, n_, 0)] = epsi;

  //----------------------------------------------------------------------------
  // The variables of the model are independently unconstrained, except for the
  // first step, where they must stay what they are; the acutators must stay
  // within boundries though. The upper and lower limits of delta are [-25, 25]
  // degrees (values in radians). Accelleration must be within [-1, 1].
  //----------------------------------------------------------------------------
  ...
  for(int i = 0; i < n_-1; ++i) {
    vars_lowerbound[IND_A(A_DELTA, n_, i)] = -0.436332;
    vars_upperbound[IND_A(A_DELTA, n_, i)] =  0.436332;
    vars_lowerbound[IND_A(A_A,     n_, i)] = -1.0;
    vars_upperbound[IND_A(A_A,     n_, i)] =  1.0;
  }

  vars_lowerbound[IND_S(S_X,    n_, 0)] = vars_upperbound[IND_S(S_X,    n_, 0)] = x;
  vars_lowerbound[IND_S(S_Y,    n_, 0)] = vars_upperbound[IND_S(S_Y,    n_, 0)] = y;
  vars_lowerbound[IND_S(S_PSI,  n_, 0)] = vars_upperbound[IND_S(S_PSI,  n_, 0)] = psi;
  vars_lowerbound[IND_S(S_V,    n_, 0)] = vars_upperbound[IND_S(S_V,    n_, 0)] = v;
  vars_lowerbound[IND_S(S_CTE,  n_, 0)] = vars_upperbound[IND_S(S_CTE,  n_, 0)] = cte;
  vars_lowerbound[IND_S(S_EPSI, n_, 0)] = vars_upperbound[IND_S(S_EPSI, n_, 0)] = epsi;
  ...
  Result res;
  res.delta = solution.x[IND_A(A_DELTA, n_, 0)];
  res.a     = solution.x[IND_A(A_A, n_, 0)];

  for(int i = 0; i < n_; ++i) {
    res.xs.push_back(solution.x[IND_S(S_X, n_, i)]);
    res.ys.push_back(solution.x[IND_S(S_Y, n_, i)]);
  }
  return res;
}
